...
| `546Ah` | `carriage_current_hold_state` | Carriage current state: `PB & 20h=1`, `PA & 02h=1`, `PB & 40h=1`. Manual says PB5 high is hold current; PA02 versus manual PB1/SPDH speed-high selector still needs schematic confirmation. |
| `5474h` | `carriage_current_drive_high_candidate` | Carriage current state: final `PB & 20h=0`, `PA & 02h=1`, `PB & 40h=1`. |
| `547Eh` | `carriage_current_drive_mid_candidate` | Carriage current state: final `PB & 20h=0`, `PA & 02h=1`, `PB & 40h=0`; used by startup home seek. |
| `5488h` | `carriage_current_drive_low_candidate` | Carriage current state: final `PB & 20h=0`, `PA & 02h=0`, `PB & 40h=1`; target of the `7007h` jump table. |
...
| Carriage phase / current | `0908h`, `51F2h`, `51F7h`, `5253h`, `5306h`, `546Ah`, `5474h`, `547Eh`, `5488h`, `7287h`, `72B3h` | Service manual Figure 2-34 identifies CPU `CO1`/`PC7` as the E01A05KA carriage gate-array `TM` input; firmware `0908h` pulses that bit when `VV62=0`. Startup `51F7h-5253h` samples the `PA bit 20h` HOME candidate while walking carriage timing tables and pulsing `TM`. `51F2h` writes carriage control state to `F003h`. The `546Ah-5488h` states match the manual's carriage-current control shape, with one unresolved label mismatch: firmware uses `PA & 02h` where the manual table labels `PB1`/`SPDH`, likely a speed-high current-select input. |
